  ...
  try {
    ...
    itemsToProcess.forEach((item: any, index: number) => {
      ...
      if (item.login && typeof item.login === 'object') {
        ...
        if (item.login.uris && Array.isArray(item.login.uris) && item.login.uris.length > 0) {
      ...
      if (mappedCredential.website && mappedCredential.username && mappedCredential.password) {
      ...
      if (!mappedCredential.website && mappedCredential.username && mappedCredential.password) {
        ...
        for (const [_key, value] of Object.entries(credentialData)) {
          const strValue = String(value).trim();
          if (
            strValue &&
            (strValue.includes('http') || strValue.includes('www.') || strValue.includes('.com'))
          ) {
...
export const parseCSVImport = (csvString: string): ImportCredential[] => {
  ...
  if (!mappedFields.includes('username') || !mappedFields.includes('password')) {
