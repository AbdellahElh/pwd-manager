...
export function handleDbError(error: unknown): Error {
  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    switch (error.code) {
      case 'P2002': // Unique constraint failed
        ...
        if (target) {
          if (target.includes('email')) {
            return ServiceError.validationFailed('A user with this email already exists');
          }
      ...
      case 'P2025': // Record not found
        if (error.message.includes('delete')) {
          return ServiceError.notFound('The record you are trying to delete was not found');
        }
        if (error.message.includes('update')) {
          return ServiceError.notFound('The record you are trying to update was not found');
        }
    ...
    return ServiceError.internal(error.message || 'An unexpected error occurred');
  }
