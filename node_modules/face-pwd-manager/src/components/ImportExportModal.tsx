...
const ImportExportModal: React.FC<ImportExportModalProps> = ({
  ...
  return (
    <div className='p-6 max-w-4xl w-full'>
          ...
          <div className='space-y-3'>
            ...
            >
              ...
              <div className='text-left'>
                ...
                <div className='text-sm text-gray-400'>Spreadsheet format, works with Excel</div>
              </div>
            </button>
          </div>
        </div>
      ...
      {activeTab === 'import' && (
        <div className='space-y-4'>
          <div className='text-sm text-gray-400 mb-4'>Import credentials from a file:</div>
          {/* Format Selection */}
          <div className='space-y-3'>
          ...
          <div className='space-y-2'>
            ...
            >
              ...
              <div className='text-xs text-gray-500 mt-1'>
                {importFormat === 'json' ? 'JSON files (.json)' : 'CSV files (.csv)'}
              </div>
            </label>
            <input
              id='import-file'
          ...
          <div className='p-3 bg-[#181818] rounded-lg border border-gray-700'>
            ...
            <div className='text-xs text-green-400 mb-3'>
              Bitwarden, LastPass, 1Password, Dashlane, Chrome, Firefox exports and more!
            </div>

            <div className='text-sm font-medium mb-2'>📋 Supported Formats:</div>
            <div className='text-xs text-gray-400 space-y-1'>
              <div>
                <strong>JSON:</strong> Bitwarden exports (items/login structure), direct arrays,
                single objects
              </div>
              <div>
            ...
            <div className='text-sm font-medium mb-2 mt-3'>📋 Supported Field Names:</div>
            <div className='text-xs text-gray-400 space-y-1'>
              <div>
                <strong>Website/URL:</strong> website, site, url, uri, login_uri, domain, host,
                server, location
              </div>
              <div>
                <strong>Username:</strong> username, user, login, email, login_username, account,
                identity
              </div>
              <div>
                <strong>Password:</strong> password, pass, login_password, pwd, secret, credential
              </div>
              <div>
                <strong>Title/Name:</strong> title, name, service, app, description, label, company
              </div>
            </div>

            <div className='text-xs text-blue-400 mt-3 p-2 bg-blue-950/30 rounded border border-blue-800'>
              <strong>Smart Import:</strong> We automatically detect Bitwarden's nested structure,
              ignore irrelevant fields, and create fallback websites when missing. Field names are
              matched flexibly!
            </div>
          </div>
          {isProcessing && (
            <div className='text-center text-blue-400 text-sm'>Processing file...</div>
          )}
        </div>
      )}
    </div>
